...
final Logger _log = Logger('PrecedenceGraph');
...
class OperandNode extends PrecedenceNode {
  ...
  @override
  void debugPrint() {
    _log.fine("|OperandNode:" + getOperandLongName(operand));
  }
  ...
  PrecedenceNode? compact() {
    ...
    if (debugLevel > 0) {
      ...
      _log.info("end node:$num");
      end.debugPrint();
    }
    if (end != this) {
      if (debugLevel > 0) {
        int num = end.node.key!.value;
        _log.info("Shortcut to node$num!");
      }
...
class PrecedenceGraph {
  ...
  void compact() {
    _log.fine("==Compact start==");
    root?.compact();
    _log.fine("==Compact end==");
  }
}
